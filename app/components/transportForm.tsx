...
export default function TransportForm({ onChoiceMade }: { onChoiceMade?: () => void }) {
  return (
    <div className='grid justify-center gap-6 md:grid-cols-2 md:gap-12'>
      {TRANSPORT_METHODS.map((method) => (
        ...
        >
          ...
          >
            <button type='submit' className='after:contents-[""] w-full bg-[#656ADD8C] bg-opacity-75 py-2 text-2xl font-bold text-white after:absolute after:inset-0 after:h-full after:w-full'>
              {method.name}
            </button>
